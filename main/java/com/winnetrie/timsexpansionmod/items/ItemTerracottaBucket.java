...
public class ItemTerracottaBucket extends ItemBase{

	/** field for checking if the bucket has been filled. */
    private final Block containedBlock;

    public ItemTerracottaBucket(String name, Block containedBlockIn)
        ...
        else
        {
            ...
            else if (flag)
            {
                ...
                else
                {
                    ...
                    if (material == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0)
                    {
                        ...
                        return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, this.fillBucket(itemstack, playerIn, ItemInit.TERRACOTTA_BUCKET_WATER));
                    }
                    else if (material == Material.LAVA && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0)
                    {
                        ...
                        worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 11);
                        playerIn.addStat(StatList.getObjectUseStats(this));
                        return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, this.fillBucket(itemstack, playerIn, ItemInit.TERRACOTTA_BUCKET_LAVA));
                    }
                    else
                    {
                        return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
            ...
            else
            {
                ...
                else if (this.tryPlaceContainedLiquid(playerIn, worldIn, blockpos1))
                {
                    ...
                    boolean flaglava = this.containedBlock==Blocks.LAVA;
                    if (flaglava) 
                    {
                    	playerIn.addStat(StatList.getObjectUseStats(this));
                    ...
                    {
                    playerIn.addStat(StatList.getObjectUseStats(this));
                    return !playerIn.capabilities.isCreativeMode ? new ActionResult(EnumActionResult.SUCCESS, new ItemStack(ItemInit.TERRACOTTA_BUCKET_EMPTY)) : new ActionResult(EnumActionResult.SUCCESS, itemstack);
                    }
                }
                else
                {
